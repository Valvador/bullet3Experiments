...
// p is point, v0, v1, v2 triangle vertices
// implemented from Christopher Erickson's 'Real Time Collision Detection', p. 141
static Vector3 closestPointBetweenPointTriangle(const Vector3& p, const Vector3& v0, const Vector3& v1, const Vector3& v2)
{
	// p in v0's voronoi region
	Vector3 edge01 = v1 - v0;
	Vector3 edge02 = v2 - v0;
	Vector3 pTo0 = p - v0;
	float d1 = edge01.dot(pTo0);
	float d2 = edge02.dot(pTo0);
	if (d1 <= 0.0f && d2 <= 0.0f)
	{
		return v0;
	}

	// p in v1's voronoi region
	Vector3 pTo1 = p - v1;
	float d3 = edge01.dot(pTo1);
	float d4 = edge02.dot(pTo1);
	if (d3 >= 0.0f && d4 <= d3)
	{
		return v1;
	}

	// p in region of edge01
	float vc = d1*d4 - d3*d2;
	if (vc <= 0.0f && d1 >= 0.0f && d3 <= 0.0f)
	{
		// Project p onto edge01
		float v = d1 / (d1 - d3);
		return v0 + edge01 * v;
	}

	// p in v2's voronoi region
	Vector3 pTo2 = p - v2;
	float d5 = edge01.dot(pTo2);
	float d6 = edge02.dot(pTo2);
	if (d6 >= 0.0f && d5 <= d6)
	{
		return v2;
	}

	// p in edge02's voronoi region
	float vb = d5 * d2 - d1 * d6;
	if (vb <= 0.0f && d2 >= 0.0f && d6 <= 0.0f)
	{
		// project p onto edge02
		float w = d2 / (d2 - d6);
		return v0 + edge02 * w;
	}

	// p in edge12's voronoi region
	float va = d3 * d6 - d5 * d4;
	if (va <= 0.0f && (d4 - d3) >= 0.0f && (d5 - d6) >= 0.0f)
	{
		// project p onto edge12
		float w = (d4 - d3) / ((d4 - d3) + (d5 - d6));
		return v1 + (v2 - v1) * w;
	}

	// p inside of face region.
	float denom = 1.0f / (va + vb + vc);
	float v = vb * denom;
	float w = vc * denom;
	return v0 + edge01 * v + edge02 * w;
}
